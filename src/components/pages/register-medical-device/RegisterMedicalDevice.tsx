...
const RegisterMedicalDevice = ({
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full flex flex-col bg-[#FCFCFC]">
          ...
          <div className="w-full flex-wrap flex-grow flex items-center">
            {devices &&
              devices.map(({ categoryName, id }) => (
                <DeviceCategory key={id} text={categoryName} />
              ))}
          </div>
        </div>
      </RegisterForm>
      ...
      <RegisterForm title="مشخصات دستگاه">
        <div className="grid grid-cols-3 gap-5 lg:px-8 px-0 pb-2">
          ...
          <div className="2xl:col-span-1 xl:col-span-2 col-span-3 h-full">
            ...
            {errors["file"] && touched["file"] && (
              // @ts-ignore
              <p className="text-borderError">{errors["file"]}</p>
            )}
          </div>
        </div>
      </RegisterForm>
      ...
      <div className="w-full flex justify-start relative">
        ...
        <label htmlFor="registerDeviceCheckbox" className="mr-2 cursor-pointer">
      ...
      <div className="w-full flex justify-center items-center mt-5">
        <div className="w-[350px] py-2">
          <button
            disabled={!(isValid && dirty && checked && file)}
            type="submit"
