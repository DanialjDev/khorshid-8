...
const PosterBox = ({
  ...
  return (
    ...
    //           pathname.startsWith("/panel/statistics") ||
    //           pathname.startsWith("/panel/medical-equipments-list")
    //             ? "py-5"
    //             : ""
    //         } items-center`}
    //       >
    //         <div
    //           className={`flex justify-center py-4 md:scale-100 scale-[0.8]`}
    //         >
    //           {icon ? (
    //             icon
    //           ) : (
    ...
    <div
      onClick={() =>
        href ? push(`/panel/medical-equipments-list${href}`) : null
      }
      className={`w-full  hover:border-primary h-full ${
        isSelected === id ? "!bg-primaryLight2 border-primary" : ""
      } hover:bg-primaryLight3 transition-all duration-200  p-2 border-2 rounded-[8px] cursor-pointer ${
